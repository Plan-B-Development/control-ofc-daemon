...
**Latest release:** v2.25.0 — 2026-09-01. Pairs with `control-ofc-gui` ≥ v2.23.0 (the recommended capability floor; the package itself only hard-blocks GUIs < 2.0.0, the sole-writer cutover). v2.42.0 or newer drives the `POST /fans/openfan/rescan` route, v2.44.0 or newer surfaces the `skipped_controls[]` field as a "Not controlled" badge, v2.45.0 or newer consumes the `control_outputs[]` field and the `controls` health subsystem, v2.49.0 or newer removes a profile search directory via `control.profile_search_dir_remove`, v2.49.2 or newer shows the thermal-forcing verify refusal as a soft notice rather than an error, and v2.49.5 or newer mirrors the widened `nct67xx` `CPUTIN` warning list this daemon acts on; older GUIs ignore or mis-word them but keep working. See [`CHANGELOG.md`](CHANGELOG.md) for the full history.
