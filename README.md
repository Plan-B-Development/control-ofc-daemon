...
**Latest release:** v2.13.0 — 2026-07-30. Pairs with `control-ofc-gui` ≥ v2.23.0 (the recommended capability floor; the package itself only hard-blocks GUIs < 2.0.0, the sole-writer cutover). See [`CHANGELOG.md`](CHANGELOG.md) for the full history.
...
```bash
paru -S control-ofc-daemon
```

**Without the AUR:** the AUR is a third-party service that goes read-only for
maintenance without warning — GitHub does not. Every release attaches the same
...
To build the package yourself from the in-repo `PKGBUILD` instead — also
AUR-free, and it does not trust a prebuilt binary:
...
> `updpkgsums` (from `pacman-contrib`) is required first: the in-repo
> `sha256sums` is regenerated by CI at tag time against the published tarball,
> so it is not guaranteed current on a fresh clone.
