...
**Latest release:** v2.22.0 — 2026-08-23. Pairs with `control-ofc-gui` ≥ v2.23.0 (the recommended capability floor; the package itself only hard-blocks GUIs < 2.0.0, the sole-writer cutover). v2.42.0 or newer drives the `POST /fans/openfan/rescan` route, v2.44.0 or newer surfaces the `skipped_controls[]` field as a "Not controlled" badge, and v2.45.0 or newer consumes the new `control_outputs[]` field and the `controls` health subsystem; older GUIs simply ignore all of them. See [`CHANGELOG.md`](CHANGELOG.md) for the full history.
