...
- Easy to test with curl: `curl --unix-socket /run/control_ofc/control_ofc.sock http://localhost/status`

## Socket Path
- Default: `/run/control_ofc/control_ofc.sock` (configurable via `ipc.socket_path` in config)
