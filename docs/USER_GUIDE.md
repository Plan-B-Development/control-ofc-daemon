...
| `POST /hwmon/{header}/characterize` | Start a PWM/RPM response sweep (DEC-313, 2.29.0+). A *deeper* diagnostic beside the ~6 s verify above, not a replacement: it holds the header at several duties and reports **command acceptance, PWM readback and physical RPM response as three separate verdicts** — collapsing them would report a pump that overrides PWM during startup as a broken fan. Returns `202`; poll `GET /diagnostics/characterization`. Optional `{"points_pct": [...], "settle_seconds": N}`, both clamped server-side. **A pump is never swept below 30%, and 0% is unreachable for any header.** |
