...
fn default_socket_path() -> String {
    "/run/control_ofc/control_ofc.sock".into()
}
...
pub struct ProfilesConfig {
    ...
    /// The GUI stores profiles at `~/.config/control_ofc/profiles/` by default.
...
fn default_profile_search_dirs() -> Vec<String> {
    let mut dirs = vec!["/etc/control_ofc/profiles".to_string()];
    if let Ok(xdg) = std::env::var("XDG_CONFIG_HOME") {
        dirs.push(format!("{xdg}/control_ofc/profiles"));
    } else if let Ok(home) = std::env::var("HOME") {
        dirs.push(format!("{home}/.config/control_ofc/profiles"));
    } else {
        // Fallback for systemd services where HOME is not set.
        // The daemon typically runs as root; /root is the standard home.
        dirs.push("/root/.config/control_ofc/profiles".to_string());
    }
    dirs
}
...
mod tests {
    ...
    fn parse_empty_toml_uses_defaults() {
        ...
        assert_eq!(config.ipc.socket_path, "/run/control_ofc/control_ofc.sock");
    }
...
[ipc]
socket_path = "/tmp/control_ofc.sock"
"#;
        ...
        assert_eq!(config.ipc.socket_path, "/tmp/control_ofc.sock");
        assert!(config.validate().is_ok());
    }
...
"#;
        ...
        assert_eq!(config.profiles.search_dirs[0], "/etc/control_ofc/profiles");
        assert_eq!(
    ...
    fn profiles_default_includes_etc() {
        ...
        assert!(
            config
                .profiles
                .search_dirs
                .contains(&"/etc/control_ofc/profiles".to_string()),
            "default search_dirs must include /etc/control_ofc/profiles"
        );
    }
...
"#;
    ...
    fn defaults_include_root_fallback_when_home_unset() {
        ...
        assert!(
            dirs.contains(&"/etc/control_ofc/profiles".to_string()),
            "must always include /etc/control_ofc/profiles"
        );
        assert!(
            dirs.contains(&"/root/.config/control_ofc/profiles".to_string()),
    ...
    fn defaults_use_home_when_set() {
        ...
        assert!(dirs.contains(&"/home/testuser/.config/control_ofc/profiles".to_string()));
        assert!(
            !dirs.contains(&"/root/.config/control_ofc/profiles".to_string()),
    ...
    fn persist_profile_search_dirs_creates_section() {
        ...
        std::fs::write(&path, "[state]\nstate_dir = \"/var/lib/control_ofc\"\n").unwrap();

        let dirs = vec![
            "/etc/control_ofc/profiles".to_string(),
            "/home/user/.config/control_ofc/profiles".to_string(),
        ];
        ...
        assert_eq!(search_dirs[0].as_str().unwrap(), "/etc/control_ofc/profiles");
    }
    ...
    fn persist_profile_search_dirs_roundtrip() {
        ...
        let dirs = vec!["/etc/control_ofc/profiles".to_string()];
        persist_profile_search_dirs(path.to_str().unwrap(), &dirs).unwrap();
